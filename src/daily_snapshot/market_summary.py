...
日本語で1文だけ、最大70字で市場の特徴を簡潔に解説してください。

厳守ルール:
- 必ず句点(。)で完結した完全な1文を返す。
- 70字を超えたら情報を削って文を完結させる(途中で切るのは禁止)。
- 銘柄名は最大2つまで(代表的なものを選ぶ)。
- ティッカー(BTC, ETH 等)はそのまま、数字を1〜2個盛り込む。
- 「〜です」「〜となっている」等の自然な日本語。
- 装飾(絵文字・見出し・改行)なし、本文のみ。

良い例(64字): BTCの建玉が19.3億ドルで前日比7.5%減少、ETHも同程度の縮小が見られ市場全体は調整局面に入っています。
"""
